...
using System.Xml;
using TerminalMACS.Util.ClassLibrary;
...
namespace TerminalMACS.Util.Extensions
{
    public static partial class Extention
    {
        ...
        public static byte[] ToSHA1Bytes(this string str, Encoding encoding)
        {
            ...
            byte[] outputBytes = shal.ComputeHash(inputBytes);
            return outputBytes;
        }
        ...
        {
            byte[] shalBytes = str.ToSHA1Bytes(encoding);
            string resStr = BitConverter.ToString(shalBytes);
        ...
        {
            return string.IsNullOrWhiteSpace(jsonStr) ? null : JsonConvert.DeserializeObject<List<T>>(jsonStr);
        }
        ...
        {
            return string.IsNullOrWhiteSpace(jsonStr) ? null : JsonConvert.DeserializeObject<DataTable>(jsonStr);
        }
        ...
        {
            return string.IsNullOrWhiteSpace(jsonStr) ? JObject.Parse("{}") : JObject.Parse(jsonStr.Replace("&nbsp;", ""));
        }
        ...
        {
            return string.IsNullOrWhiteSpace(jsonStr) ? JArray.Parse("[]") : JArray.Parse(jsonStr.Replace("&nbsp;", ""));
        }
        ...
        public static T ToEntity<T>(this string json)
        {
            ...
            foreach (var item in type.GetProperties())
            {
                ...
                string patterm = "\"" + item.Name + "\":\"(.*?)\"";
                foreach (Match match in Regex.Matches(json, patterm))
                {
        ...
        public static IPEndPoint ToIPEndPoint(this string str)
        {
            ...
            IPEndPoint iPEndPoint = new IPEndPoint(IPAddress.Parse(addr), port);
            return iPEndPoint;
        }
